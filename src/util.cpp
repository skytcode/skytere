...
const char * const BITCOIN_CONF_FILENAME = "skytere.conf";
const char * const BITCOIN_PID_FILENAME = "skytered.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "skytered -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SkytereCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SkytereCore
    // Mac: ~/Library/Application Support/SkytereCore
    // Unix: ~/.skyterecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SkytereCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SkytereCore";
#else
    // Unix
    return pathRet / ".skyterecore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty skytere.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Skytere Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Skytere Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Skytere Core developers";
    }
