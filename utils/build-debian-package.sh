...
PACKAGE_VERSION=$($SRC_PATH/src/skytered --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/skytered $DEB_BIN
cp $SRC_PATH/src/skytere-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/skytere-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/skytered.1 $DEB_MAN
cp $SRC_DOC/man/skytere-cli.1 $DEB_MAN
cp $SRC_DOC/man/skytere-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/skytered.bash-completion $DEB_CMP/skytered
cp $SRC_PATH/contrib/skytere-cli.bash-completion $DEB_CMP/skytere-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/skytered.1
gzip --best -n $DEB_MAN/skytere-cli.1
gzip --best -n $DEB_MAN/skytere-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/skytered $DEB_BIN/skytere-cli
