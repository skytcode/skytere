...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/SkytereParams"
else
    PARAMS_DIR="$HOME/.skytere-params"
fi
...
DOWNLOAD_URL="https://github.com/skytcode/skytere/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with SKYT_DISABLE_SOMETHING=1
SKYT_DISABLE_WGET="${SKYT_DISABLE_WGET:-}"
#SKYT_DISABLE_IPFS="${SKYT_DISABLE_IPFS:-}"
SKYT_DISABLE_CURL="${SKYT_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$SKYT_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$SKYT_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$SKYT_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Skytere parameters!
...
main() {
    ...
    cat <<EOF
SKYT - fetch-params.sh

This script will fetch the Skytere SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Skytere SNARK parameters. Note that it is
