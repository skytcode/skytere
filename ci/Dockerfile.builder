...
# skytere_hash
RUN git clone https://github.com/skytcode/skytere_hash
RUN cd skytere_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} skytere
RUN useradd -u ${USER_ID} -g skytere -s /bin/bash -m -d /skytere skytere
...
RUN mkdir /skytere-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /skytere-src && \
