...
*Setup instructions for a Gitian build of Skytere Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Skytere
...
- [Building Skytere Core](#building-skytere-core)
...
Clone the git repositories for Skytere Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/skytcode/skytere
...
in the Skytere Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Skytere Core
----------------

To build Skytere Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Skytere Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/skytere/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/skytcode/skytere
...
For example:
```bash
URL=https://github.com/crowning-/skytere.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit skytere=${COMMIT} --url skytere=${URL} ../skytere/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit skytere=${COMMIT} --url skytere=${URL} ../skytere/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit skytere=${COMMIT} --url skytere=${URL} ../skytere/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the skytere git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../skytere/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/skytere
SIGPATH=/some/root/path/skytere-detached-sigs

./bin/gbuild --url skytere=${BTCPATH},signature=${SIGPATH} ../skytere/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/skytere-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/skytere-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/skytere-osx-build.assert
```
...
[skytere/gitian.sigs](https://github.com/skytcode/gitian.sigs/) repository, or if that's not possible to create a pull
request.
